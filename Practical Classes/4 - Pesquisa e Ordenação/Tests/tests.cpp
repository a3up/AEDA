...
EXPECT_THROW(p1
.getFrequencia("Tiago Silva"), ClienteNaoExistente);
try {
p1.getFrequencia("Tiago Silva");
}
catch (
ClienteNaoExistente &e
) {
cout << "Apanhou excecao. Cliente nao existente: " << e.

getNome()

<<
endl;
EXPECT_EQ("Tiago Silva", e.
...
p1.entrar("Rui Silva");
p1.entrar("Susana Costa");
cout << "Este teste nunca falha! VERIFICAR informacao escrita no monitor" <<
endl;
cout <<
p1;
...
EXPECT_THROW(p1
.getClienteAtPos(6), PosicaoNaoExistente);
try {
p1.getClienteAtPos(6);
}
catch (
PosicaoNaoExistente &e
) {
cout << "Este teste nunca falha. Verifique no monitor a informacao" <<
endl;
cout << "Apanhou excecao. Posicao nao existente:" << e.
